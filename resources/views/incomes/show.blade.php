            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="text-center mb-6 pb-6 border-b border-gray-200">
                        ...
                        <p class="text-4xl font-bold text-green-600">{{ format_currency($income->amount) }}</p>
                    </div>
                    ...
                    <dl class="divide-y divide-gray-200">
                        <div class="py-4 sm:grid sm:grid-cols-3 sm:gap-4">
                            ...
                            <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{ $income->title }}</dd>
                        </div>

                        <div class="py-4 sm:grid sm:grid-cols-3 sm:gap-4">
                            <dt class="text-sm font-medium text-gray-500">{{ __('messages.Date') }}</dt>
                            <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{ $income->date->format('F d, Y') }}</dd>
                        </div>
                        ...
                        @if ($income->reference_number)
                            <div class="py-4 sm:grid sm:grid-cols-3 sm:gap-4">
                                ...
                                <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{ $income->reference_number }}</dd>
                            </div>
                        @endif
                        ...
                        @if ($income->notes)
                            <div class="py-4 sm:grid sm:grid-cols-3 sm:gap-4">
                                ...
                                <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{ $income->notes }}</dd>
                            </div>
                        @endif

                        <div class="py-4 sm:grid sm:grid-cols-3 sm:gap-4">
                            <dt class="text-sm font-medium text-gray-500">{{ __('messages.Created At') }}</dt>
                            <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{ $income->created_at->format('F d, Y h:i A') }}</dd>
                        </div>

                        @if ($income->updated_at != $income->created_at)
                            <div class="py-4 sm:grid sm:grid-cols-3 sm:gap-4">
                                <dt class="text-sm font-medium text-gray-500">{{ __('messages.Last Updated') }}</dt>
                                <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{ $income->updated_at->format('F d, Y h:i A') }}</dd>
                            </div>
                        @endif
                    </dl>
                    ...
                    <div class="mt-6 pt-6 border-t border-gray-200 flex items-center justify-between">
                        ...
                        <form action="{{ route('incomes.destroy', $income) }}" method="POST" onsubmit="return confirm('{{ __('messages.Are you sure you want to delete this income?') }}');">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
