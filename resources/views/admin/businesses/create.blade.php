    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <div class="flex items-center space-x-4">
                ...
                <div>
                    <h2 class="font-bold text-xl text-gray-800 leading-tight">{{ __('messages.create_new_business') }}</h2>
                    <p class="text-sm text-gray-500">{{ __('messages.create_assign_business') }}</p>
                </div>
            </div>
        </div>
    </x-slot>
                ...
                <div class="px-6 py-5 border-b border-gray-100 bg-gradient-to-r from-indigo-50 to-purple-50">
                    <div class="flex items-center">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-500">{{ __('messages.fill_business_info') }}</p>
                        </div>
                    </div>
                </div>

                <form action="{{ route('admin.businesses.store') }}" method="POST" class="p-6 space-y-6">
                    @csrf
                    ...
                    <div>
                        ...
                        <textarea name="address" id="address" rows="3"
                            class="w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 transition"
                            placeholder="{{ __('messages.enter_business_address') }}">{{ old('address') }}</textarea>
                        @error('address')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <!-- Assign to User -->
                    <div>
                        <label for="user_id" class="block text-sm font-medium text-gray-700 mb-2">{{ __('messages.assign_to_user') }} ({{ __('messages.business_admin') }}) *</label>
                        <select name="user_id" id="user_id" required
                            ...
                            @empty
                                <option value="" disabled>{{ __('messages.no_unassigned_users') }}</option>
                            @endforelse
                        </select>
                        @error('user_id')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        @if($unassignedUsers->isEmpty())
                            <p class="mt-2 text-sm text-yellow-600 flex items-center">
                                ...
                                {{ __('messages.all_users_assigned') }}
                            </p>
                        @endif
                    </div>
                        ...
                        <div class="flex space-x-4">
                            <label class="flex items-center p-4 bg-green-50 rounded-xl border-2 cursor-pointer transition hover:bg-green-100
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-green-600">{{ __('messages.business_active_immediately') }}</p>
                                </div>
                            </label>
                            <label class="flex items-center p-4 bg-yellow-50 rounded-xl border-2 cursor-pointer transition hover:bg-yellow-100
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-yellow-600">{{ __('messages.requires_approval') }}</p>
                                </div>
                            </label>
                        </div>
                        @error('status')
