...
<div class="py-12">
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
        <div class="mb-8">
            <h2 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('messages.Subscribe') }} - {{ $business->name }}</h2>
            <p class="mt-2 text-gray-600 dark:text-gray-400">Choose a plan and payment method to subscribe</p>
        </div>

        @if($currentSubscription)
        <div class="mb-6 p-4 bg-blue-50 dark:bg-blue-900/50 border border-blue-200 dark:border-blue-800 rounded-lg">
            <p class="text-sm text-blue-700 dark:text-blue-300">
                <strong>Note:</strong> You currently have an active subscription ({{ $currentSubscription->plan->name }})
                that expires on {{ $currentSubscription->ends_at->format('M d, Y') }}.
                Subscribing to a new plan will replace your current subscription.
            </p>
        </div>
        @endif

        <form action="{{ route('subscriptions.store', $business) }}" method="POST">
            @csrf

            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 mb-6">
                <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Select a Plan</h3>
            ...
            <div class="flex justify-end space-x-4">
                <a href="{{ route('subscriptions.show', $business) }}"
